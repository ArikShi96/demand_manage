...
class SearchPanel extends Component {
  ...
  getOpenText = (searchOpen) => {
    let openText = "展开", openIcon = 'uf uf-arrow-down';
    if (searchOpen) {
      openText = '收起';
  ...
  render() {
    ...
    let { children, bgColor, reset, searchName, hideSearch, style, advanced } = this.props;
    let _style = style || {};
    return (
      <div className="ucg-search-panel" style={{ background: bgColor, ..._style }}>
        <div className="ucg-search-btn">
          {hideSearch ? null : <Button colors="dark" onClick={this.search}>{searchName}</Button>}
          {reset ? <Button className="button-transparent" onClick={this.reset}>清空</Button> : null}
          {advanced ? (
