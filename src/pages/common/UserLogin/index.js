...
class UserLogin extends Component {
  constructor(props) {
    super(props);
    let cookieUserName =
      process.env.NODE_ENV === "development" ? "----" : Cookie.get("userName");
  ...
  getDropDownOverlay = () => {
    ...
    return (
      <Menu className="mix-user-name-menu" onSelect={this.handleMenuSelect}>
        {menus.map((menu) => {
        ...
        {navUser ? (
          <Menu.Item key="userInfo">
            <i className="uf uf-home" />
            个人信息
          </Menu.Item>
        ) : null}
        <Menu.Item key="logout">
          <i className="uf uf-back" />
          退出
        </Menu.Item>
      </Menu>
    );
