import { FormControl, Button, Select, Switch, Pagination, Table, Message, Icon, Modal } from 'tinper-bee';
...
class SearchModel extends React.Component {
    ...
    columns = [
        { title: "发布人", dataIndex: "publisherName", key: "publisherName", width: '10%' },
        { title: "预算", dataIndex: "projectBudget", key: "projectBudget", width: '10%' },
        {
        ...
        {
            title: "所属领域", dataIndex: "domain", key: "domain", width: '10%',
            render: (value) => {
                return this.domainstyle(value)
            }
        },
        { title: "联系人", dataIndex: "contactMan", key: "contactMan", width: '10%' },
        { title: "联系方式", dataIndex: "contactPhone", key: "contactPhone", width: '15%' },
        {
            title: "推荐", dataIndex: "recommend", key: "recommend", width: '10%',
            render: (value, record) => {
                return (
                    <Switch
        ...
        {
            title: "操作", dataIndex: "operation", key: "operation", width: '20%',
            render: (value, record, index) => {
                return <Fragment>
                    <a onClick={this.showDetail.bind(null, record.id, record.verifyStatus)}>查看</a>
                    {
    ...
    getInitList(current = 0, ps = 10) {
        ...
        fetch(myapi.BASE_URL + `/market/requirement/manage/getList?pn=${current}&ps=${ps}&isAjax=1`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json;charset=UTF-8'
            },
            ...
            .then((response) => {
                ...
                if (response.status === 0) {
                    ...
                    if (response.status === 1) {
                        ...
                        allCarouse.sort((a, b) => {
                            return new Date(b.bidEndtime).getTime() - new Date(a.bidEndtime).getTime()
                        });
    ...
    pullBlackFun() {
        let that = this
        fetch(myapi.BASE_URL + `/market/requirement/blacklist/${this.state.blackIds}/add`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json;charset=UTF-8'
            },
            ...
            if (data.status === 0) {
                Message.create({ content: '已在黑名单中，请勿重复操作', color: 'danger' });
                that.setState({
                    pullBlackState: false
                })
            } else {
    ...
    OperationState(e, ids, index) {
        if (e === 2) {
            return (<div className="a-box">
                <a onClick={this.managefun.bind(this, "approve", ids, index, 'downshelf')}>通过</a>
            ...
            return (<div className="a-box">
                <a onClick={this.managefun.bind(this, "approve", ids, index, 'upshelf')}>上架</a>
                <a onClick={this.managefun.bind(this, "delete", ids, index)}>删除</a>
                <a onClick={this.pullBlack.bind(this, ids, index)}>拉黑</a>
            </div>)
        }
    ...
    searchlist() {
        ...
        fetch(myapi.BASE_URL + `/market/requirement/manage/getList?contactPhone=${phones}&isAjax=1&ps=10&verifyStatus=${stid}&domainId=${domid}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json;charset=UTF-8'
            },
                ...
                <Content style={{ width: '100%' }}>
                                ...
                                >
                                    <Option value="0">全部</Option>
                                    {
                                        this.statusBox.map((item, index) => (
                                            <Option key={index} value={item.id}>{item.stat}</Option>
                                        ))
                                    }
                                </Select>
                            </FormList.Item>
                                ...
                                >
                                    <Option value="">全部</Option>
                                    {
                                        this.alldomainbox.map((item, idx) => (
                                            <Option key={idx} value={item.id}>{item.domainName}</Option>
                                        ))
                                    }
                                </Select>
                            </FormList.Item>
                    ...
                    >
                        ...
                        <Modal.Footer>
                            ...
                            <Button onClick={this.refusefun.bind(this)} colors="primary">确认</Button>
                        </Modal.Footer>
                    </Modal>
                    ...
                    >
                        ...
                        <Modal.Footer>
                            ...
                            <Button onClick={this.pullBlackFun.bind(this)} colors="primary">确认</Button>
                        </Modal.Footer>
                    </Modal>
                </Content>
