...
class IsvDetail extends React.Component {
      ...
      .then((response) => {
        if (response.status === 1) {
          ...
          let basicInfo = [{
            ...
            title: "统一社会信用代码",
            value: response.basInfo ? "basInfo.unitLice" : "data.licenceNumber"
          }, {
            title: "所属行业",
            value: response.basInfo ? "basInfo.businessType" : "data.industry"
          }, {
            title: "企业注册地址",
          ...
          let personInfo = [{
            title: "法定代表人姓名",
            value: response.basInfo ? "basInfo.legalName" : "data.legalPerson"
          }, {
            title: "法人证件类型",
  ...
  render() {
    let { allValue, basicInfo, personInfo, isNew, partnerType, partnerBackup, chanpayAccount, chanpayBackup } = this.state;
    return (
      <Fragment>
        <Header style={{ background: '#fff', padding: 0 }} title="服务商详情" back />
          ...
          <div className="isv-detail">
            <div className="isv-detail-zone">
              <div className="isv-detail-zone-row">
                ...
                <Button className="isv-detail-zone-btn" style={{ marginRight: 0 }} colors="dark" onClick={this.enable}>{allValue.data.status === "1" ? "停用服务商" : "启用服务商"}</Button>
              </div>
              {
                basicInfo.length > 0 ?
                  basicInfo.map((item) => {
              ...
              <div className="isv-detail-zone-item">
                <div className="isv-detail-zone-item-title">伙伴类型</div>
                {
                  partnerType ?
                    ...
                    : <div className="isv-detail-zone-item-value">{this.renderType(allValue.data.partnerType)}</div>
                }

                {
                  partnerType ?
                    <Fragment>
                      <Button className="isv-detail-zone-item-edit" colors="dark" onClick={this.save.bind(null, "partnerType")}>保存</Button>
                      <Button className="isv-detail-zone-item-edit" colors="secondary" onClick={this.hide.bind(null, "partnerType")}>取消</Button>
                    </Fragment>
                    :
                    <Button className="isv-detail-zone-item-btn" bordered onClick={this.show.bind(null, "partnerType")}>修改</Button>
                }
              </div>
              <div className="isv-detail-zone-item">
                <div className="isv-detail-zone-item-title">企业营业执照图片</div>
                <img className="isv-detail-zone-item-value" src={isNew ? allValue.basInfo.custCertPath : allValue.data.licencePic} />
              </div>
            ...
            <div className="isv-detail-zone">
              <div className="isv-detail-zone-row">
                ...
                <div className="isv-detail-zone-header">企业法人及经办人信息</div>
              </div>
              {
                personInfo.length > 0 ?
                  personInfo.map((item) => {
              ...
              <div className="isv-detail-zone-item">
                <div className="isv-detail-zone-item-title">畅捷支付账户</div>
                {
                  chanpayAccount ?
                    ...
                    />
                    : <div className="isv-detail-zone-item-value">{allValue.data.chanpayAccount}</div>
                }
                {
                  chanpayAccount ?
                    <Fragment>
                      <Button className="isv-detail-zone-item-edit" colors="dark" onClick={this.save.bind(null, "chanpayAccount")}>保存</Button>
                      <Button className="isv-detail-zone-item-edit" colors="secondary" onClick={this.hide.bind(null, "chanpayAccount")}>取消</Button>
                    </Fragment>
                    :
                    <Button className="isv-detail-zone-item-btn" bordered onClick={this.show.bind(null, "chanpayAccount")}>修改</Button>
                }
              </div>
            </div>
          </div>
