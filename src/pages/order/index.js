import { FormControl, Button, Select, Pagination, Table, Tabs } from 'tinper-bee';
...
class SearchModel extends React.Component {
    ...
    columns = [
        {
            title: "订单编号", dataIndex: "agreementNum", key: "agreementNum", width: "14%",
            render: (value) => {
                return <a onClick={this.goDetail.bind(null, value)}>{value}</a>
            }
        },
        { title: "商品名称", dataIndex: "productName", key: "productName", width: "10%" },
        {
            title: "服务商", dataIndex: "isvName", key: "isvName", width: "12%",

        },
        { title: "客户名称", dataIndex: "operatorName", key: "operatorName", width: "10%", },
        { title: "订单原价", dataIndex: "originalPrice", key: "originalPrice", width: "8%" },
        { title: "折扣", dataIndex: "discount", key: "discount", width: "8%" },
        { title: "支付方式", dataIndex: "payMode", key: "payMode", width: "8%" },
        { title: "应付金额", dataIndex: "busiAmount", key: "busiAmount", width: "8%" },
        {
            title: "推荐码", dataIndex: "recommendCode", key: "recommendCode", width: "6%",
            render: (value) => {
                return this.renderField(value, "recommendCode")
            }
        },
        {
            title: "下单时间", dataIndex: "commitTime", key: "commitTime", width: "7%",
            render: (value) => {
    ...
    searchlist = (page = 0, size = 10, billStatus = "") => {
        let { startTime, endTime, notFree, recommendCode, orderStatus,payMode,productName, operatorName, siteId, isvName, agreementNum } = this.state;
        fetch(myapi.BASE_URL + `/market/yonyoucloud/allOrders?agreementNum=${agreementNum}&isvName=${isvName}&startTime=${startTime}&endTime=${endTime}
    ...
    //渲染顶部tab页
    getTabPane = () => {
        let { dataSource, activePage, statusCode, statusPaid, recommendCode, notFree,
            orderStatus, statusOrder, statusFree, isvName, agreementNum, suitStatus,
            siteId,payMode,payModeCode,operatorName,productName } = this.state;
        return (
            statusPaid.map(tabPane => {
                let { stat, id } = tabPane;
                return (
                    <TabPane tab={stat} key={id}>
                        ...
                        <div style={{ height: '32px', margin: '10px 0' }}>
                            <Button colors="primary" style={{ float: 'right', marginRight: '20px' }} onClick={this.jsonToExcel}>列表导出</Button>
                        </div>
